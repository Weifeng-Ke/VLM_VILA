...
class LlavaMetaForCausalLM(ABC):
    '''Adding new flag `use_image_embed_cache` to enable image embedding cache during inference'''
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.enable_prefix_cache = False  # default off
    ...
    ''' This is a new added method'''
    def _get_image_embed_cache(self):
        """Lazy-init image embedding cache (used only at inference)."""
        if not hasattr(self, "_image_embed_cache"):
            # key -> Tensor[num_image_tokens, hidden_dim]
            self._image_embed_cache = {}
        return self._image_embed_cache
    ''' This is a new added method'''
    def clear_prefix_cache(self):
        """
        Clear all cached image embeddings used for prefix caching.
        Safe to call anytime between inference runs to reset memory.
        """
        if hasattr(self, "_image_embed_cache"):
            cache_size = len(self._image_embed_cache)
            self._image_embed_cache.clear()
            print(f"[PrefixCache] Cleared {cache_size} cached image embeddings.")
        else:
            print("[PrefixCache] No cache found to clear.")
    ...
    def __embed_media_tokens(
        ...
        return embeds
    '''
    '''I replace the old method with a new method below to enable image embedding cache during inference'''
    '''def __embed_media_tokens(
        self,
        media: Dict[str, List[torch.Tensor]],
        media_config: Dict[str, Dict[str, Any]],
        ...
        for name in media:
            ...
            if self.training:
                ...
                # Training: original behavior (no caching)
                embeds[name] = deque(self.encoders[name](media[name], media_config[name]))
                continue

            # ---------------------------------
            # 2) INFERENCE PATH WITH CACHING
            # ---------------------------------
            if not getattr(self, "enable_prefix_cache", False):
                embeds[name] = deque(self.encoders[name](media[name], media_config[name]))
                continue
            if name == "image":
                # Lazy-init cache
                cache = self._get_image_embed_cache()

                image_embeds_list = []
                # media[name] is a List[Tensor]; each tensor is one preprocessed image
                for img in media[name]:
                    # Build a deterministic key for this image tensor.
                    # We use shape + dtype + a small content fingerprint.
                    # This is fast and collisions are extremely unlikely.
                    key = (
                        tuple(img.shape),
                        str(img.dtype),
                        float(img.mean().item()),
                    ...
                    if key in cache:
                        # Reuse cached embedding: Tensor[num_image_tokens, hidden_dim]
                        image_embeds_list.append(cache[key])
                    else:
                        # Run encoder on a single-image list to get a one-element list of embeddings
                        # encoders["image"] returns List[Tensor[num_image_tokens, hidden_dim]]
                        embeds_single_list = self.encoders[name]([img], media_config[name])
                        assert len(embeds_single_list) == 1, "Expected one embedding per image tensor"
                        embeds_single = embeds_single_list[0]

                        # Cache it
                        cache[key] = embeds_single
                        image_embeds_list.append(embeds_single)

                # Maintain original type: deque of 2D tensors
                embeds[name] = deque(image_embeds_list)
            else:
                # Non-image media (e.g. video) → original behavior
                embeds[name] = deque(self.encoders[name](media[name], media_config[name]))

        return embeds
        '''
    ''' End of new method'''
    ''' Trying the full image content caching below'''
    def __embed_media_tokens(
        self,
        media: Dict[str, List[torch.Tensor]],
        media_config: Dict[str, Dict[str, Any]],
        ...
        """
        Encode media tokens (images/videos) into embeddings.

        This version adds content-based caching for image embeddings during inference.
        Each image tensor is hashed by its raw content (SHA256 of tensor bytes).
        """
        embeds = defaultdict(deque)
        ...
        def _tensor_hash(tensor: torch.Tensor) -> str:
            """
            Compute SHA256 hash of a tensor's content on CPU.
            This ensures that identical images (even if reloaded) share the same key.
            """
            t = tensor.detach().to("cpu", non_blocking=True).contiguous()
            # Cast to float32 to make hash stable across dtype differences (fp16 vs fp32)
        ...
        for name in media:
            ...
            # ---------------------------------
            if not getattr(self, "enable_prefix_cache", False):
                # Cache disabled → run normal encoder path
                embeds[name] = deque(self.encoders[name](media[name], media_config[name]))
                continue

            # -------------------------------------------------
            # Image caching with content-based SHA256 hashing
            # -------------------------------------------------
            if name == "image":
                cache = self._get_image_embed_cache()
                image_embeds_list = []
                ...
                for img in media[name]:
                    img_hash = _tensor_hash(img)  # unique hash of content

                    if img_hash in cache:
                        # Cache hit → reuse embeddings
                        image_embeds_list.append(cache[img_hash])
                    else:
                        # Compute embeddings and cache
                        embeds_single_list = self.encoders[name]([img], media_config[name])
                        assert len(embeds_single_list) == 1, "Expected one embedding per image tensor"
                        embeds_single = embeds_single_list[0]
                        cache[img_hash] = embeds_single
                        image_embeds_list.append(embeds_single)
